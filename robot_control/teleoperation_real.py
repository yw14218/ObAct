...
def move_arm(bot, goal_pos, goal_rot, gripper_status):
    ...
    if positions is not None:
        bot.arm._publish_commands(positions=positions.position[:6], moving_time=1, accel_time=0.3, blocking=False)
...
if __name__ == "__main__":
    ...
    # bot.arm.moving_time = 1
    # bot.arm.accel_time = 1
    ...
    while True:
        if ready_to_execute.is_set():
            if udp_server.latest_cmd is not None:
                ...
                goal_pos = goal_pos - base_pos + initial_gripper_pose[:3,3]
                goal_rot = goal_rot - base_rot + R.from_matrix(initial_gripper_pose[:3,:3]).as_euler("xyz")
                ...
                move_arm(bot, goal_pos, goal_rot, gripper_status)
